    ...
    <div class="flex justify-between items-center">
        ...
        <ul class="hidden md:flex space-x-6 text-sm font-medium items-center">
            ...
            @foreach ($links as $route => $label)
                <li>
                    <a href="{{ route($route) }}" class="relative inline-block px-3 py-2 group transition-all duration-300 ease-in-out">
                        <span class="absolute inset-0 bg-blue-500 opacity-0 group-hover:opacity-20 blur-md rounded-full transition-all duration-300"></span>
            ...
            <li>
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="relative inline-block px-3 py-2 group transition-all duration-300 ease-in-out">
        ...
        <div class="md:hidden" x-data="{ open: false }">
            ...
            <div x-show="open" x-transition
                ...
                @foreach ($links as $route => $label)
                    <a href="{{ route($route) }}" class="relative block px-4 py-2 group transition-all duration-300 ease-in-out">
                        <span class="absolute inset-0 bg-blue-500 opacity-0 group-hover:opacity-20 blur-md rounded-md transition-all duration-300"></span>
                        <span class="relative z-10">{{ $label }}</span>
                    </a>
                @endforeach
                <form method="POST" action="{{ route('logout') }}">
